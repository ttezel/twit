...
OARequest.prototype.persist = function () {
  ...
  this.parser.on('element', function (msg) {
    ...
    else if (msg.friends)         { self.emit('user_friends', msg) }
    else if (msg.event)           {
      switch(msg.event) {
        // reference: https://dev.twitter.com/docs/streaming-apis/messages#User_stream_messages
        case 'blocked': self.emit('ustream_blocked', msg) break
        case 'unblocked': self.emit('ustream_unblocked', msg) break
        case 'favorite': self.emit('ustream_favorite', msg) break
        case 'unfavorite': self.emit('ustream_unfavorite', msg) break
        case 'follow': self.emit('ustream_follow', msg) break
        case 'unfollow': self.emit('ustream_unfollow', msg) break
        case 'user_update': self.emit('ustream_user_update', msg) break
        case 'list_created': self.emit('ustream_list_created', msg) break
        case 'list_destroyed': self.emit('ustream_list_destroyed', msg) break
        case 'list_updated': self.emit('ustream_list_updated', msg) break
        case 'list_member_added': self.emit('ustream_list_member_added', msg) break
        case 'list_member_removed': self.emit('ustream_list_member_removed', msg) break
        case 'list_user_subscribed': self.emit('ustream_list_user_subscribed', msg) break
        case 'list_user_unsubscribed': self.emit('ustream_list_user_unsubscribed', msg) break
        default: self.emit('user_event', msg) break
      }
    }
