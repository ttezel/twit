...
Twitter.prototype = {
  ...
  request: function (method, path, params, callback) {
    ...
    return new OARequest(this.auth, method, this.checkForRetweet(path, params) + '.json', this.normalizeParams(path, params)).end(callback)
  },
  ...
  normalizeParams: function (path, params) {
    if(path != REST_ROOT + 'statuses/retweet'){
  ...
  checkForRetweet: function(path, params) {
    if (path === REST_ROOT + 'statuses/retweet'){
      path += '/'+params
    };
    return path
