...
/* the easy way to do statuses/update_with_media:   

    // body has binary Buffer of PNG
    // post{} has other variables to post
    ...
    T.post('statuses/update_with_media', T.makeForm(payload), ...);
 */
  makeForm: function(payload) {
    var boundary, form, key, i, pair;

    boundary = '-------------------------' + Math.random().toString().substr(2);

    form = 'Content-Type: multipart/form-data; boundary=' + boundary;
    for (i = 0; i < payload.length; i++) {
      pair = payload[i];

      form += '\r\n--' + boundary + '\r\n';
      for (key in pair.headers) {
        if (pair.headers.hasOwnProperty(key)) form += key + ': ' + pair.headers[key] + '\r\n';
      }
      form += '\r\n' + pair.value;
    }
    form += '\r\n--' + boundary + '--\r\n';
