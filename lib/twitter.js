...
Twitter.prototype._buildReqOpts = function (method, path, params, isStreaming, callback) {
  ...
  if (isStreaming) {
    ...
    if (path === 'media/upload') {
      ...
      // special case for account/update_profile_image
      if (path === 'account/update_profile_image') {
        reqOpts.headers['content-type'] = 'multipart/form-data; start="<image>"'
        reqOpts.multipart = [ {
          'content-location': 'image',
          'content-disposition': 'form-data; name="image"',
          'content-transfer-encoding': 'binary',
          'content-id': '<image>',
          body: Buffer(finalParams.image, 'base64') }
        ];
