...
var Auth = require('./auth')
  ...
  , querystring = require('querystring') // added by rodrigopolo.com
...
Twitter.prototype = {
  ...
  request: function (method, path, params, callback) {
    ...
    // added by rodrigopolo.com
    if(path==REST_ROOT + 'statuses/retweet' || path==REST_ROOT + 'statuses/destroy'){
      finalPath = finalPath + '/' + params.id + '.json';
      delete finalParams.id;
    }else if(path==REST_ROOT + 'favorites/create' || path==REST_ROOT + 'favorites/destroy'){
      // this.path = path + (params ? '?' + querystring.stringify(params) : '')
      finalPath = finalPath + '.json?' + querystring.stringify(params);
      delete finalParams.id;
      delete finalParams.include_entities;
    }else{
      finalPath = finalPath + '.json'; // modified the return finalPath
    }
    // end - added by rodrigopolo.com

    return new OARequest(this.auth, method, finalPath, finalParams).end(callback)
  },
