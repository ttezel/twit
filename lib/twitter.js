...
Twitter.prototype._buildReqOpts = function (method, path, params, isStreaming, callback) {
  ...
  // clone `params` object and keep object integrity, so we can modify it without modifying the user's reference
  var paramsClone = require('util')._extend(params)
  ...
  if (isStreaming) {
    ...
    if (path === 'media/upload') {
      ...
      if(path === 'account/update_profile_image' || path === 'account/update_profile_background_image') {
        reqOpts.headers['Content-type'] = 'multipart/form-data'
